...
def read_frame(frame_info,avg_size):
    for i in frame_info :
        ...
        if i[3] == 1 :
            save_frameName = './tmp/ex/'+ '1/' + edit_name + '.png'

        elif i[3] == 2:
            save_frameName = './tmp/ex/' + '2/' + edit_name + '.png'

        frame = cv2.imread(str(i[0]))
        dst_frame = crop_bbox(frame, i[2])
        save_frame(dst_frame, save_frameName,avg_size)

def crop_bbox(frame,bbox):
    result = frame[bbox[1]:bbox[1]+bbox[3], bbox[0]:bbox[0]+bbox[2]]
    return result

def save_frame(frame, frame_name,avg_size) :
    print(frame_name)
    size = ((int(avg_size[2])), (int(avg_size[3])))

    try :
        frame = cv2.resize(frame, dsize=size, interpolation=cv2.INTER_AREA)
