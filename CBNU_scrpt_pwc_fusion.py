...
# im0_fn = 'data/S153.png';
# im1_fn = 'data/S154.png';
# im2_fn = 'data/S155.png';
# flow_fn = './tmp/S154_fusion.flo';
def search(dirName):
    ...
    for filename in filenames:
        ...
        if fileext == '.png':
            listnames.append(int(os.path.splitext(filename)[0]))

    listnames.sort()

    for box in range(0, len(listnames) - 1, 3):
        img1 = dirName + '/' + str(listnames[box]) + '.png'
        img2 = dirName + '/' + str(listnames[box+1]) + '.png'
        img3 = dirName + '/' + str(listnames[box+2]) + '.png'
        flow_fn = dirName + '/' + str(listnames[box+1]) +'_fusion' + '.flo'
        pwc_fusion(img1,img2,img3,flow_fn)
...
def pwc_fusion(im0_fn,im1_fn,im2_fn, flow_fn):
...
search('/home/gim/Downloads/PWC-Net/Multi_Frame_Flow/tmp/ex/notcrop_2')